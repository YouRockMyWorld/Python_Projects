...
def Process_Sheet(sheet):
    ...
    for row in rows_list:
        ...
        result = [row[0],row[1],row[5]*1000,row[4]*1000,row[8]*1000,rows_list[aim_index][5]*1000,rows_list[aim_index][4]*1000,rows_list[aim_index][8]*1000,row[9],row[10], row[6]]
        if row[6] == rows_list[aim_index][6]:
            continue
        pointstr = ''
        if(row[0]<row[1]):
            pointstr = row[0]+row[1]
        else:
            pointstr = row[1]+row[0]
        if pointstr not in processed_set:
